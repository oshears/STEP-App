...
class SwiftPopTipView: UIView {
    ...
    func presentPointingAtBarButtonItem(barButtonItem: UIBarButtonItem, animated: Bool) {
        let targetView = barButtonItem.valueForKey("view") as! UIView
    ...
    func presentPointingAtView(targetView: UIView, containerView: UIView, animated: Bool) {
        ...
        if self.dismissTapAnywhere {
            self.dismissTarget = UIButton(type: .Custom)
            self.dismissTarget?.addTarget(self, action: #selector(SwiftPopTipView.dismissTapAnywhereFired(_:)), forControlEvents: .TouchUpInside)
        ...
        if animated {
            if self.animation == .Slide {
                ...
                UIView.setAnimationDidStopSelector(#selector(SwiftPopTipView.popAnimationDidStop(_:finished:context:)))
    ...
    func dismissAnimated(animated: Bool) {
        ...
        if animated {
            ...
            self.frame = frame
            UIView.setAnimationDelegate(self)
            UIView.setAnimationDidStopSelector(#selector(SwiftPopTipView.finaliseDismiss))
            UIView.commitAnimations()
        } else {
    ...
    func autoDismissAnimatedDidFire(theTimer: NSTimer) {
        let animated = theTimer.userInfo?.objectForKey("animated") as! Bool
    ...
    func autoDismissAnimated(animated: Bool, atTimeInterval timeInvertal:NSTimeInterval) {
        ...
        self.autoDismissTimer = NSTimer.scheduledTimerWithTimeInterval(timeInvertal, target: self, selector: #selector(SwiftPopTipView.autoDismissAnimatedDidFire(_:)), userInfo: userInfo, repeats: false)
    }
